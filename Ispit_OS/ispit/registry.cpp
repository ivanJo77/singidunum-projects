...
DWORD Registry::_getValueAsString(HKEY key, const LPWSTR subKey, const LPWSTR value, LPWSTR buffer, DWORD bufferSize)
{
     ...
     (type == REG_SZ || type == REG_EXPAND_SZ))
  {
    ...
    {
      DWORD i = (size / sizeof(WCHAR)) - 1; //Gets the index of the last character.
      
      //The last character is \0, then the size is equal to the position of the symbol.
    ...
    if(size > 2/*min. variable size is 3 characters*/ && type == REG_EXPAND_SZ)
    {
