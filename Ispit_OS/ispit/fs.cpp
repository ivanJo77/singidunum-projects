...
{
  WCHAR path[MAX_PATH];
  DWORD i = CWA(kernel32, GetTempPathW)(MAX_PATH - 14/*retrieve GetTempFileName*/, path);
...
{
  WCHAR path[MAX_PATH];
  DWORD i = CWA(kernel32, GetTempPathW)(MAX_PATH - 14/*requirement GetTempFileName*/, path);
...
bool Fs::_setRandomFileTime(DWORD startTime, DWORD reservedSeconds, LPWSTR fileName, DWORD upLevel)
{
  ...
  if(currentTime < startTime)return false; /*paranoid*/
...
bool Fs::_createDirectoryTree(LPWSTR path, LPSECURITY_ATTRIBUTES securityAttributes)
{
  ...
  //It is unclear whether it can return NULL. Remembering the focus of the Indians with wsprintf, to protect against this.
  if(p == NULL)p = path; 
  ...
  for(;; p++)if(*p == '\\' || *p == '/' || *p == 0)
  {
    ...
    *p = 0;
    ...
    //If you got to the end, then successful exit.
