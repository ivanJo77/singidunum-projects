...
bool PeImage::_setNtHeader(PEDATA *pedata, PENTBASEDATA *basedata)
{
  ...
  if(p)
  {
    //* - values ​​that will be changed after the file build.
...
DWORD PeImage::_buildImage(PEDATA *pedata, DWORD flags, DWORD rvaOfEntryPoint, LPBYTE *output)
{
  ...
  //CSections.
  {
    ...
    //Align to the lowest position in sections.
...
void *PeImage::_copyModuleToProcess(HANDLE process, void *image)
{
	...
	return _copyModuleToProcessEx(process,image,&relocsDir,0,(IMAGE_BASE_RELOCATION *)((LPBYTE)image+relocsDir.VirtualAddress));
}

void *PeImage::_copyModuleToProcessEx(HANDLE process, void *image, IMAGE_DATA_DIRECTORY *relocsDir, DWORD relocDelta, IMAGE_BASE_RELOCATION *relHdrSrc)
{
#if defined _WIN64
  PIMAGE_NT_HEADERS64 ntHeader = (PIMAGE_NT_HEADERS)((LPBYTE)image + ((PIMAGE_DOS_HEADER)image)->e_lfanew);
#else
      ...
      if(relocsDir->Size > 0 && relocsDir->VirtualAddress > 0)
      {
        ...
        while(relHdrSrc->VirtualAddress != 0)
        {
          ...
          if(relHdrSrc->SizeOfBlock >= sizeof(IMAGE_BASE_RELOCATION))
          {
            ...
            LPWORD relList = (LPWORD)((LPBYTE)relHdrSrc + sizeof(IMAGE_BASE_RELOCATION));
...
{
  //Search for a section that owns the address.
  DWORD ret = 0;
...
# endif
  ...
  //get NT-header.
  //Here we test to determine the main parameters of 32/64 and to prevent "buffer overflow".
  //Make a more rigorous test does not see the point.
...
#   if(PEIMAGE_32 > 0)
    case IMAGE_FILE_MACHINE_I386:
      ...
      if(/*dosHeader->e_lfanew >= memSize - sizeof(IMAGE_NT_HEADERS32) || Verification is carried out above.*/nt32Header->OptionalHeader.Magic != IMAGE_NT_OPTIONAL_HDR32_MAGIC)return false;
...
#   endif
  ...
  //This check is required to protect against buffer overflow attacks in progress with sections.
